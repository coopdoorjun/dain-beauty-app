...
const simulateMobileFetch=async()=>{
...
function KpiCard({label,value,color=T.text,sub,icon,gradient,onRefresh}:{label:string;value:string;color?:string;sub?:string;icon?:string;gradient?:string;onRefresh?:()=>void}){
  ...
  return(
    <div style={{...glass,padding:"20px 22px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{fontSize:11,color:T.sub,marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
        ...
        {onRefresh&&(
          <button onClick={handle} style={{marginLeft:"auto",background:"transparent",border:"1px solid "+T.border2,color:T.sub,padding:"2px 8px",borderRadius:6,fontSize:9,cursor:"pointer",display:"flex",alignItems:"center",gap:3}}>
            {loading?"⏳":"🔄"} 실시간
          </button>
        )}
      </div>
...
export default function App(){
  ...
  const SidebarContent=()=>(
    <>
      ...
      <nav style={{flex:1,padding:"10px 0",overflowY:"auto"}}>
        {NAV.map(item=>(
          <div key={item.id} onClick={()=>goTab(item.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"14px 24px",cursor:"pointer",fontSize:13,color:tab===item.id?T.text:T.sub,background:tab===item.id?"rgba(255,51,102,0.08)":"transparent",borderLeft:"3px solid "+(tab===item.id?T.point:"transparent"),transition:"all 0.15s"}}>
            <span style={{color:tab===item.id?T.point:T.dim,fontSize:16}}>{item.icon}</span>
      ...
      <div style={{padding:"12px 18px",borderTop:"1px solid "+T.border}}>
        <button onClick={()=>setLoggedIn(false)} style={{width:"100%",padding:"9px",background:"rgba(255,51,102,0.08)",border:"1px solid rgba(255,51,102,0.2)",borderRadius:8,color:T.point,fontSize:11,cursor:"pointer",fontWeight:700}}>로그아웃</button>
      </div>
    </>
  );
  ...
  return(
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"Pretendard,-apple-system,system-ui,sans-serif"}}>
      ...
      {/* 모바일 드로어 오버레이 */}
      {isMobile&&drawerOpen&&(
        <div onClick={()=>setDrawerOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:998,backdropFilter:"blur(2px)"}}/>
      )}

      {/* 모바일 드로어 */}
      {isMobile&&(
        <aside style={{position:"fixed",top:0,left:0,height:"100vh",width:280,background:T.surface,borderRight:"1px solid "+T.border,display:"flex",flexDirection:"column",zIndex:999,transform:drawerOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.3s cubic-bezier(0.4,0,0.2,1)",boxShadow:drawerOpen?"4px 0 32px rgba(0,0,0,0.7)":"none"}}>
          <SidebarContent/>
        </aside>
      )}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        {/* 모바일 상단 헤더 */}
        {isMobile&&(
          <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(7,7,7,0.95)",backdropFilter:"blur(10px)",borderBottom:"1px solid "+T.border,padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
            <button onClick={()=>setDrawerOpen(true)} style={{background:"transparent",border:"1px solid "+T.border2,borderRadius:8,color:T.text,padding:"8px 10px",cursor:"pointer",fontSize:18,lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
              ☰
            </button>
            <Logo/>
            <div style={{marginLeft:"auto",fontSize:10,color:T.sub}}>{NAV.find(n=>n.id===tab)?.label}</div>
            {totalAlerts>0&&(
              <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",minWidth:18,height:18,borderRadius:9,background:GS.pink,color:"#fff",fontSize:9,fontWeight:900,padding:"0 5px"}}>{totalAlerts}</span>
            )}
          </header>
        )}
        ...
        <main style={{flex:1,overflowY:"auto",padding:isMobile?"20px 16px":"36px 40px"}}>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28,flexWrap:"wrap" as const,gap:10}}>
                ...
                <div style={{display:"flex",gap:8",flexWrap:"wrap" as const}}>
                  ...
                  <RefreshBtn loading={!!loadingCard["all"]} onClick={()=>refreshCard("all",()=>setKpi({sales:"₩"+fmt(rand(2800000,4200000)),cvr:(2+Math.random()*1.5).toFixed(1)+"%",roas:(5+Math.random()*3).toFixed(1)+"x"}))} label="⚡ 즉시 새로고침"/>
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:12,marginBottom:20}}>
                <KpiCard label="오늘 매출" value={kpi.sales} color={T.green} sub="전일비 +12.4%" icon="💰" gradient={GS.green} onRefresh={()=>setKpi(p=>({...p,sales:"₩"+fmt(rand(2800000,4200000))}))}/>
                <KpiCard label="전체 CVR" value={kpi.cvr} color={T.teal} sub="전일비 +0.5%p" icon="⚡" gradient={GS.teal} onRefresh={()=>setKpi(p=>({...p,cvr:(2+Math.random()*1.5).toFixed(1)+"%"}))}/>
                <KpiCard label="평균 ROAS" value={kpi.roas} color={T.gold} sub="목표 5.0x 초과" icon="📈" gradient={GS.gold} onRefresh={()=>setKpi(p=>({...p,roas:(5+Math.random()*3).toFixed(1)+"x"}))}/>
                <KpiCard label="미처리 긴급" value={totalAlerts+"건"} color={totalAlerts>0?T.point:T.green} sub="CS · 재고 · 공고" icon="🔔" gradient={GS.pink}/>
              </div>
          ...
          {tab==="store"&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28,flexWrap:"wrap" as const,gap:10}}>
                <div><h1 style={{margin:0,fontSize:isMobile?18:22,fontWeight:900,letterSpacing:1}}>🛒 스마트스토어 관리</h1><p style={{margin:"6px 0 0",fontSize:12,color:T.sub}}>네이버 스마트스토어 · 5분 단위 동기화</p></div>
              ...
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(3,1fr)",gap:12,marginBottom:20}}>
                <KpiCard label="오늘 매출" value="₩3,420,000" color={T.green} sub="전일비 +12.4%" gradient={GS.green} onRefresh={()=>setKpi(p=>({...p,sales:"₩"+fmt(rand(2800000,4200000))}))}/>
              ...
              <div style={{...glass,padding:24,marginBottom:18}}>
                ...
                <div style={{overflowX:"auto" as const}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:400}}>
                    <thead><tr style={{borderBottom:"1px solid "+T.border2}}>{["기간","매출","전기 대비","성장률","객단가"].map(h=><th key={h} style={{padding:"10px 14px",textAlign:"left",fontWeight:500,color:T.sub,fontSize:10,letterSpacing:0.8,whiteSpace:"nowrap" as const}}>{h}</th>)}</tr></thead>
                  ...
                  {CS_FEED.map((c,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"13px 0",borderBottom:"1px solid "+T.border}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",gap:6,marginBottom:4,alignItems:"center",flexWrap:"wrap" as const}}><Badge color={c.urgent?T.point:T.dim}>{c.ch}</Badge><span style={{fontSize:11,fontWeight:700}}>{c.user}</span><span style={{fontSize:10,color:T.dim,marginLeft:"auto"}}>{c.time}</span></div>
          ...
          {tab==="inventory"&&(
            <div>
              <div style={{marginBottom:28}}><h1 style={{margin:0,fontSize:isMobile?18:22,fontWeight:900,letterSpacing:1}}>📦 재고 & 목표 관리</h1></div>
              <div style={{...glass,padding:24,marginBottom:18}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap" as const,gap:8}}>
                ...
                {[{label:"전체 매출",pct:72,color:T.point},{label:"세라마이드 토너",pct:85,color:T.blue},{label:"리바이브 세럼",pct:48,color:T.warn}].map((bar,i)=>(
                  <div key={i} style={{marginBottom:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:6,color:T.sub}}><span>{bar.label}</span><span style={{color:bar.color,fontWeight:700}}>{bar.pct}%</span></div>
                ...
                <div style={{...glass,padding:24}}>
                  ...
                  <div style={{overflowX:"auto" as const}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:360}}>
                      <thead><tr style={{borderBottom:"1px solid "+T.border2}}>{["품목명","현재고","일 판매량","예상 품절","상태"].map(h=><th key={h} style={{padding:"10px 8px",textAlign:"left",fontWeight:500,color:T.sub,fontSize:10,letterSpacing:0.8,whiteSpace:"nowrap" as const}}>{h}</th>)}</tr></thead>
                        ...
                        <tr key={i} style={{borderBottom:"1px solid "+T.border}}>
                          ...
                          <td style={{padding:"13px 8px"}}>
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              <div style={{width:36,height:5,background:T.dim,borderRadius:2}}><div style={{width:Math.min(item.daily*8,100)+"%",height:"100%",background:item.status==="품절임박"?T.point:T.green,borderRadius:2}}/></div>
                  ...
                  {INVENTORY.map((item,i)=>{
                    ...
                    return(
                      <div key={i} style={{background:T.card2,border:"1px solid "+T.border,borderRadius:12,padding:14,marginBottom:12}}>
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:6}}><span style={{color:T.sub}}>마진율</span><span style={{color:margin>=40?T.green:margin>=25?T.warn:T.point,fontWeight:700}}>{margin}%</span></div>
          ...
          {tab==="social"&&(
            <div>
              <div style={{marginBottom:28}}><h1 style={{margin:0,fontSize:isMobile?18:22,fontWeight:900,letterSpacing:1}}>📱 소셜 통합 관리</h1><p style={{margin:"6px 0 0",fontSize:12,color:T.sub}}>{SOCIAL_ID} · TikTok / Instagram / YouTube</p></div>
              ...
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:12,marginBottom:20}}>
                <KpiCard label="팔로워" value={SOCIAL_DATA[socialPlatform].followers} color={SOCIAL_DATA[socialPlatform].color} gradient={GS.teal} onRefresh={()=>{}}/>
                <KpiCard label="7일 증감" value={SOCIAL_DATA[socialPlatform].followerGrowth} color={T.green} gradient={GS.green} onRefresh={()=>{}}/>
                <KpiCard label="수집 영상" value={SOCIAL_DATA[socialPlatform].videos.length+"개"} gradient={GS.blue}/>
                <KpiCard label="총 조회수" value={SOCIAL_DATA[socialPlatform].videos.reduce((a,v)=>a+parseFloat(v.views)*(v.views.includes("K")?1000:1000000),0).toLocaleString()} gradient={GS.gold} onRefresh={()=>{}}/>
              </div>
              <div style={{...glass,padding:24,marginBottom:18}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap" as const,gap:8}}>
              ...
              <div style={{borderTop:"1px solid "+T.border,paddingTop:28}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:20,flexWrap:"wrap" as const,gap:10}}>
                  <div><h2 style={{margin:0,fontSize:isMobile?16:18,fontWeight:800}}>🎬 숏폼 전략실</h2><p style={{margin:"6px 0 0",fontSize:12,color:T.sub}}>AI 대본 · 해시태그 · 촬영 체크리스트</p></div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
          ...
          {tab==="funding"&&(
            <div>
              <div style={{marginBottom:28}}><h1 style={{margin:0,fontSize:isMobile?18:22,fontWeight:900,letterSpacing:1}}>🏛️ 정부지원사업 스케줄러</h1><p style={{margin:"6px 0 0",fontSize:12,color:T.sub}}>기업마당 · 경기기업비서 · 창업진흥원</p></div>
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)",gap:12,marginBottom:20}}>
                {GOV_SOURCES.map((s,i)=>(
